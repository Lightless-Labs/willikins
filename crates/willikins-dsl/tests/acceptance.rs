//! Parses the milestone 1 reference documents and checks them against
...
fn read_fixture(relative: &str) -> String {
    ...
    std::fs::read_to_string(format!("{path}{relative}"))
...
fn new_rust_service_checks_against_the_fake_catalog() {
    let source = read_fixture("workflows/new-rust-service.yaml");
    let workflow = parse_document(&source).expect("the positive fixture parses");
...
fn secret_into_template_fails_check_with_exactly_one_taint_error() {
    let source = read_fixture("workflows/fixtures/secret-into-template.yaml");
    let workflow = parse_document(&source).expect("the negative fixture parses");
