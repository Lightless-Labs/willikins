...
| `config_get_inheritable_true.json` | `GET /v3/configs/config` (200, `inheritable: true`) | **hand-assembled from the config object shape** the research note quotes ("Config Inheritance": the reference example shows `"inheritable": true` on a `GET`'s response), not recorded live — the sandbox is on a Team plan (confirmed 2026-09-14) but no live run has exercised this endpoint yet. Recording the two real `POST` response bodies live in a throwaway project is open work, not done here |
| `config_get_inherits_present.json` | `GET /v3/configs/config` (200, `inherits: [{project, config}]`) | same caveat as `config_get_inheritable_true.json`: hand-assembled from the documented shape, project values as project *names* per the research note's own correction, not yet recorded live |
| `config_get_inherits_extra.json` | `GET /v3/configs/config` (200, `inherits` holding one entry beyond what a test asks for) | same caveat; used by `config_inherits_ensure_mock.rs` and `fake_agrees_with_live.rs` to pin the `Mismatch` reasoning in `doppler.config.inherits.ensure`'s own module doc |
