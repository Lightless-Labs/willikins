...
| `service_token_post_created.json` | `POST /v3/configs/config/tokens` | **not recorded on purpose**: the response carries a live token. The proof is that the typed parse into `DopplerServiceToken` succeeds against the real endpoint, which it did. Since 2026-09-16 the file's own `key` is the placeholder `DOPPLER_TOKEN_PLACEHOLDER`, not a token-shaped literal — every reader (`service_token_ensure_mock.rs`, `service_token_rotate_mock.rs`) substitutes in a `concat!`-assembled, pattern-valid token before use, so this file on disk never carries a real detector's shape |
...
| `config_get_inheritable_true.json` | `GET /v3/configs/config` (200, `inheritable: true`) | **confirmed live 2026-09-21**: a config this run had just marked inheritable answered exactly this, alongside `inheriting`, `inherits` and `inheritedBy`, which no tool reads |
| `config_get_inheritable_false.json` | `GET /v3/configs/config` (200, `inheritable: false`) | **confirmed live 2026-09-21**: this is what a freshly created root config answers — Doppler emits `inheritable`, `inheriting`, `inherits` and `inheritedBy` on every config body, so `false` (not an omitted key) is the real "not inheritable" shape, and `config_get_present.json`'s omission models an older, narrower recording. It exists because `config_get_present.json` omits the key entirely, and with only that body in the suite, weakening `doppler.config.inheritable.ensure`'s read from `== Some(true)` to `.is_some()` left every test green (mutation, 2026-09-20). Used by `config_inheritable_ensure_mock.rs` and `fake_agrees_with_live.rs` |
| `config_get_inherits_present.json` | `GET /v3/configs/config` (200, `inherits: [{project, config}]`) | **confirmed live 2026-09-21**: a config made to inherit one base answered `"inherits": [{"project": "<project name>", "config": "<config name>"}]` and `"inheriting": true`, and the base's own `inheritedBy` named it back. The `project` value is the project **name**, as the research note's correction says — which is what lets `ConfigRefBody` deserialize it straight into a `DopplerProject` |
...
- **`doppler.service_token.ensure` and `.rotate` failed to plan against a project that does not
  ...
  **What that 404 cannot settle**, recorded so it is not re-derived: Doppler documents no
  ...
  that really is unreachable fails one call later, at the mint `POST`. At the time this was
  fixed the tolerance was kept one status wide on purpose: a `400` — this API's *other* answer
  for a project that is not there, recorded under "Undocumented facts the cycle settled" below —
  still refused the plan, because whether this endpoint ever answered it was never observed.
...
- **`doppler.project.ensure` (and every other Doppler read needing a project) failed to plan
  ...
  The coordinator's first probe read the difference as timing (quiescence), the reading the
  solution note's filename carried until it was renamed. A second probe the same day, this time
  ...
  Nothing about elapsed time enters into it: a third probe sampled the same absent name seven
  times either side of a create and a delete, from one second out to five minutes, and the
  answer flipped with the token's view at the first sample and never drifted back.
  ...
  **at plan time, both answers mean "this token cannot see a project by this name right now"**,
  and the create call (or, for `doppler.secret.get`, apply time) is left as the only arbiter of
  whether that was actually because the resource does not exist — the same deferral
  `doppler.project.ensure::ensure` already relies on by re-reading after a failed create rather
  than parsing its error body. `willikins_providers_doppler::client::looks_like_a_missing_project`
  is the one predicate every affected read now shares, and it stays exactly one *message* wide:
  a `400` naming anything else — Doppler's *other* `400`, "Project name already exists in this
  workplace." on a duplicate create foremost among them — still fails, pinned in every affected
  mock-test file by a test using that exact phrasing. Full write-up:
