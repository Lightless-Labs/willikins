...
**Addendum:** 2026-09-14 (night, task 10a) — `willikins-server` library landed in two halves and an Opus verify (Workflow `wf_a2ae26fe-894`; gates green at 1,452 tests). Landed: `Butler` (`plan`, `approve`, `reject`, `apply`, `run`, `runs`, `pending_approvals`, `validate`, `describe`, `list_workflows`, `list_tools`, `propose_slug`), a validated `DocumentSha256` in the journal's `PlanRecorded`, a `Clock` shared by the `Butler` and both journals, a narrower `Append` trait so a run thread locks the journal per append, both windows computed from the journal's own timestamps, `Approval::Human` built only from the journaled grant for that plan, decision finality, the single-apply lock (`Mutex<Option<RunId>>`), startup validation of the trusted directory (symlinks refused; a file stem must equal the document's name), per-principal token buckets, `ServerConfig::from_vars`, `live_catalog_with` (the doppler crate's assembly test now calls it), and the CLI/Butler JSON parity test (test 11's library half). The verify found and fixed three things: an approval decision was not final under concurrency (two deciders read `Pending` under separate journal locks, so a grant racing a rejection revived it; `decide` now holds one journal guard across read, checks and append, with the clock read before the guard), a `RunInProgress` refusal was not journaled (new `ApplyRefusedReason::RunInProgress { run_id }`), and `applied` was read before the lock was taken (now re-read under it). Plan corrections: the journal's `ApplyRefused` reason list is `UnknownPlan`, `DocumentChanged`, `Drift`, `PlanFailed { error_kind }` (task 5), `PlanExpired`, `ApprovalRequired`, `AlreadyApplied`, `RunInProgress { run_id }` (task 10a); and the MCP tool table's `validate` result (`errors: [CheckError]`) contradicted test 11 (every returned error carries `kind` and `message`): decided for test 11, so task 10b changes `ValidateResponse` to carry each error and warning through `Reported` (the CLI already does), and the parity test that pins the difference flips. Carried into 10b: a plan recorded before a process restart cannot be applied, because the `Butler` keeps resolved inputs in memory (`Value` has no `Deserialize`); inputs are never secret, so 10b folds them back from the journal through the type registry, since a redeploy between `plan` and a human's approval is the normal case this plan promises to survive. Carried into pass 2: every view folds the whole entry list and `apply` folds twice per call (a cached fold); a hand-edited `fingerprint` line replays as truth (operator-level). Both write cycles left `Http` unable to expose a DELETE's status code; not needed.
**Addendum:** 2026-09-15 — deployment target and its exposure. The operator created Railway project `Willikins` (service `willikins` from the GitHub repository, Dockerfile builder, auto-deploy on push) and decided, given that milestone 2's authentication is static bearer tokens plus Basic auth, that the service gets **no public domain** until a stronger auth path lands (milestone 3: short-lived or OAuth-issued credentials, narrowed provider credentials). Task 12 therefore proves that the image builds, that startup refuses without configuration, and that Railway's internal `/healthz` check passes; `WILLIKINS_ALLOWED_HOSTS` names the private domain only. Task 14's smoke run drives `serve --stdio` or a localhost `serve --http` with the sandbox credentials, not the Railway service. The two provider credentials reach Railway only through Doppler's Railway integration. Railway holds no database: the journal is one JSONL file on the service's single volume.
**Addendum:** 2026-09-15 (task 10b) — the `willikins-server` binary landed in three sonnet steps and an Opus verify (Workflow `wf_48ec5145-138`; gates green at 1,551 tests). Landed: plans survive a restart (`Butler::apply` rebuilds the resolved inputs from `PlanRecorded.inputs` through the type registry, refusing with `RecordedInputUnreadable` when a recorded value no longer parses; a property test pins `parse(render(v)) == v` for every registered non-secret type at its canonical example); `ValidateResponse` errors and warnings carry `kind` and `message` through `Reported` (the MCP tool table's `errors: [CheckError]` is to be read that way); the rmcp tool surface (`WillikinsHandler`, the eight tools of the table, `Json<T>` results, `structured_error` for domain errors), `serve --stdio` with `--principal` (default `local`) and `--fake` (announced in `initialize`'s instructions on both transports); the Streamable HTTP transport under axum (`/mcp` stateless with JSON responses, `/healthz`, `/approvals`), bearer auth with SHA-256 and a constant-time equal-length compare, agent principals `agent-<12 hex of the hash>`, Basic auth on the page with the username as the approver's principal (refused when it fails the grammar or starts with `agent-`), single-use nonces expiring with the approval window, Origin/Referer checks, the 30 s timeout, the 1 MiB body cap, seven distinct startup refusals, `tracing` JSON to stderr, `serve --http --bind`. Protocol version: `get_info` advertises `V_2026_07_28`, but no handshake can echo it (SEP-2567 moved negotiation for that version to per-request HTTP metadata), so the in-process rmcp client negotiates `2025-11-25`; pinned. Verify item 5: legacy session mode needs an `Mcp-Session-Id` on every non-initialize POST, which is why the server runs stateless. The verify found and fixed four things: `TokenHash`'s derived `Debug` printed every configured hash (now the placeholder); a zero rate limit panicked inside `spawn_blocking` (now `RateLimited`); `POST /approvals` fell back to axum's 2 MiB default instead of the configured cap; `--fake` was silent over HTTP. 31 attack tests in `crates/willikins-server/tests/adversarial_10b.rs`. Carried into pass 2: `AuthFailedReason` needs variants for a bad nonce, a foreign origin and a malformed username (all `InvalidCredential` today); `PlanRecorded` carries no principal, so the page's requester reads `unknown` after a restart; `PlanFailed { error_kind: "Unavailable" }` is not a real `PlanError` kind; any agent token may apply a plan another agent requested; a nonce posted against the wrong plan burns that plan's nonce; the origin parser is not IPv6-bracket-aware. Carried into task 11: `ButlerError::Check` and `Input` elements without `message`; the CLI's hand-maintained error-name table. Housekeeping: the every-document parity sweep will hit the 60-per-minute read bucket at 60 shipped documents.
**Addendum:** 2026-09-15 (task 11) — the CLI landed in two sonnet steps and an Opus verify (Workflow `wf_6c54999e-c23`; gates green). Library groundwork: `willikins_journal::replay(path) -> ReplayedJournal`, a lock-free validated replay for external readers (no `Journal` or `Append` impl, read views only; safe because `FileJournal::append` writes one whole line and syncs before returning), so `runs` and `run` work beside a running server, which holds the exclusive lock (a plan defect: the CLI section read `approve`, `reject`, `runs`, `run` as tools against a live server's journal, but acceptance test 10 forbids a second exclusive open; `approve` and `reject` therefore refuse with the lock error while a server holds the journal, and are the CLI-only flow's); `ButlerError::Check` and `Input` elements carry per-element messages through `Reported` (`InputError` gained `Display`; `CheckError::kind()` replaced the CLI's hand-maintained table); `willikins_server::cli::run_serve(&ServeArgs)` is the one `serve` implementation both binaries call. CLI decisions: `apply <file>` copies the file into a private temp directory as `<document name>.yaml` and starts a `Butler` there (a symlinked file argument is followed, deliberately: the operator named the path); `apply --plan-id <id> --journal <path> [--workflows-dir <dir>]` closes the approve-by-id loop and validates the real directory; `--principal` (default `local`) names the operator for both roles, and CLI self-approval is by design; `apply` polls the run to its final state; `--fake-state-out <file>` dumps the fake providers' redacted end state so a second process converges (`Unchanged`/`Converged`); `--live` builds the live catalog from the two environment variables and refuses, kind-tagged, before any network call. The verify fixed two defects: `Reported`'s `serde(flatten)` wrote `message` twice for every error type that declared its own `message` field (`CliError`, `LiveCredentialError`, `StartupError` at top level, `ButlerError::Journal`; renamed to `error`, and the CLI wraps a startup failure as `ButlerError::Startup`), and `--fake-state-out` wrote an unloadable dump on a refusal. 15 attack tests in `crates/willikins-cli/tests/adversarial_11.rs`. Disclosure from the first step: two commits sat on a red workspace gate for a few minutes before their reconciliation commit (crate-scoped gates had passed; the full gate caught the parity break). Carried into pass 2: `wait_for_run` polls without bound if the run thread's `RunFinished` append fails after `RunStarted`; `DocumentErrorKind`'s own `message` field still collides with `Reported` (a DSL wire change); rmcp round-trips for the four remaining apply refusal kinds are pinned by type identity only.
**Addendum:** 2026-09-15 (task 12) — deployment landed (Workflow `wf_e5f8cbdb-a0f`; gates green at 1,645 tests). The `Dockerfile` sits at the repository root (Railway's default path, so no `RAILWAY_DOCKERFILE_PATH`): cargo-chef on `rust:1.97-slim-bookworm`, `gcr.io/distroless/cc-debian12` at runtime, the binary plus exactly the two positive documents under `/app/workflows` (pinned by a test that computes the image's contents from the tree through `.dockerignore`), `CMD ["serve","--http"]`. `WILLIKINS_FAKE_CATALOG=1` is the environment equivalent of `--fake` so the image stays the real server and the deployment decides; any other value refuses. `hash-token` on both binaries reads a token from stdin and prints its SHA-256, refusing empty input, embedded newlines and control characters, so a real token never touches a command line. Railway: one volume (`willikins-volume`, 5000 MB, mounted at `/data`), five non-secret variables set (`WILLIKINS_JOURNAL_PATH`, `WILLIKINS_ALLOWED_HOSTS` as the private-domain reference, `WILLIKINS_FAKE_CATALOG`, and two hashes of discarded throwaway tokens), one `railway up --ci` that built and deployed on the first attempt; the service's builder self-corrected from Railpack to Dockerfile once the file existed, and the two FAILED deployments before it were GitHub-triggered Railpack builds from earlier pushes, not attempts. `.railway/railway.ts` is authored (healthcheck `/healthz`, one replica, the volume by name, no `source`, no `domains`) and not applied: the installed CLI has no `railway config` commands, IaC treats omission as deletion, and the file omits the volume's region, which must be read from the dashboard before any apply. `deploy/teardown.sh` reads the repository and project names from `willikins run <id> --json`, refuses a resource without willikins' marker, dry-runs by default, passes the Doppler token to curl through `--config -` on stdin, and refuses (deletes nothing) when a run recorded a repository but no project. The verify fixed three things: the script could not tear down a failed run (the CLI exits 1 for a failed run's record; it now decides from the document), reported a refused Doppler delete as done (`--fail` added), and `hash-token` accepted control characters. Verify item 6: distroless is accepted by Railway's pipeline (built and ran); PR-environment volumes stay undocumented, moot since the repository takes no PRs; Doppler sync latency waits for the integration. Open on the Railway side: `/healthz` has not been exercised live (`healthcheckPath` is null until the IaC file is applied or the dashboard sets it; the service has no domain), the private-network `Host` header carries a port (pinned as admitted by the allowed-hosts rule), and the service serves the fake catalog until the operator connects Doppler's Railway integration and removes the variable. Railway's start command overrides the image `ENTRYPOINT`, so the `ENTRYPOINT`/`CMD` split gates nothing beyond operator trust, recorded. Carried into pass 2: the crash-recovery detour (volume detach and reattach) is untested; a truncated last journal line refuses startup by design (truncate-and-warn versus refuse is pass 2's question).
**Addendum:** 2026-09-15 (task 13, adversarial pass 2) — pass 2 ran end to end over TCP against the real binary (Workflow `wf_7515decb-fd7`: an Opus attacker, then an Opus completeness critic; gates green at 1,703 tests; `docs/research/2026-09-15-e2e-http-adversarial-pass-2.md`). Headline: no attack reached a secret byte, forged an approval, ran an unapproved plan, or escaped the trusted directory; authentication held over TCP against every shape a client library will not send. Two availability defects fixed: `template.render` could allocate about 390 MB from 128 KiB of bounded input (now refused arithmetically before allocating), and one hung provider read filled the blocking pool while `/healthz` kept answering (now at most 64 concurrent MCP tool calls hold a blocking thread, `try_acquire` never `acquire`, a kind-tagged `Busy` beyond that; `run_status`, `list_tools` and `propose_slug` stay unbounded; the single-apply slot is `Idle`/`Preparing`/`Running` with its mutex held only to change it, reset on every exit including a panic, and a second apply during the first one's checks is `ApplyPreparing`). The journal's wire format changed additively, the one task allowed to: `PlanRecorded.principal` (optional; the approvals page names the requester after a restart), `AuthFailedReason::{InvalidNonce, ForeignOrigin, MalformedUsername}`, and `ApplyRefusedReason::RecordedInputUnreadable { input }` replacing the invented `PlanFailed { error_kind: "Unavailable" }`; a journal frozen before the change (`crates/willikins-journal/tests/fixtures/pre-pass-2-every-event.jsonl`) replays through both readers, and the critic froze the new shapes (`post-pass-2-new-shapes.jsonl`) as the next pass's baseline. Also fixed: a wrong or cross-plan nonce no longer burns the right one; `serve_http` logs the address it bound, not the one asked for; the CLI's `wait_for_run` stops when the run thread is gone. Decisions: any agent principal may apply any recorded plan (the plan is fixed at plan time, re-verified at apply, both principals journaled); a trailing-whitespace bearer token authenticates (HTTP's own field-value rule); a truncated last journal line keeps refusing, with `willikins journal repair` (`todos/2026-09-15-journal-repair-subcommand.md`) as the recorded answer; slowloris is pinned `#[ignore]`, not fixed, until a public listener exists; INFO is the log ceiling and `RUST_LOG` is not read. The critic found two handed-over items the pass skipped (task 11's rmcp round-trips of the refusal kinds, task 12's crash-recovery detour: both attacked, both held), one vacuous test and two latently vacuous sweeps (rewritten with liveness assertions), and a harness defect that turned a regression into a hang (fixed); it changed no source file, and its seven-mutation table is in the note. Five plan defects are fixed in place below (the `ApplyRefused` reason list, trust boundary 5's path template, the blocking-pool risk entry, acceptance test 19's unnamed bound, the silent log level). Handed to milestone 3 or 2c: bound the header read, an IPv6-aware origin check, the log-level story with a sweep of rmcp's own debug/trace output, the permit moved into the blocking closure, validating tool outputs against declared types and a distinguishable redaction marker, measuring the journal fold before caching it, a `journal_version` field, `willikins journal repair`.
**Addendum:** 2026-09-16 (task 14, part A) — the live smoke run split in two so nothing is created before it can be deleted. Part A (Workflow `wf_ba182dd5-636`, opus; gates green at 1,704 tests) ran the two read-only probes with the sandbox file sourced in the same command (GitHub: 3 pass, 1 skip, the org still has no repository; Doppler: the credential authenticated, the project checks 404 by design in the empty workplace) and wrote acceptance test 18 as code: `crates/willikins-cli/tests/live_smoke.rs`, behind a `live-tests` feature plus `WILLIKINS_LIVE_TESTS=1`, `#[ignore]`d, driving the real `willikins` binary through the positive fixture twice (`slug=willikins-smoke`, `org=Willikins-Test`, one persistent `--journal`, `--principal smoke-operator`), the rotation refused without `--approve` and applied with it, `deploy/teardown.sh` as a dry run then `--yes`, and a leftover check independent of the script; a pre-flight inside the test refuses to create anything unless both sandbox credentials are set, `jq` is on `PATH`, and the repository does not already exist. Its twin `tests/smoke_parity.rs` drives the same four invocations against the fake catalog in every gate, so the live run is the first time only the providers differ. Two corrections to this plan's words, found by driving the binary: on the first apply `configs[dev]`, `configs[stg]` and `configs[prd]` read `Unchanged`, not `Created`, because Doppler creates the three root configs with the project and `doppler.config.ensure` re-observes them `Present` (`changed: false`), exactly as the Doppler section above predicts (the fake agrees); and on the second apply `ci_secret` reads `Converged`, not `Unchanged`, because its `value` input is `Unknown` (a minted token cannot be re-read) and the planned action is a no-op. Acceptance test 18 is amended in place below. Part B, the live run itself, needs nothing from the operator since the teardown was re-authenticated (see the next addendum); the recorded fixtures are refreshed by the two provider write cycles, not by the smoke test, and their 2026-09-14 verification stands unless part B's run shows a new shape.
**Addendum:** 2026-09-16 (task 14, teardown re-authenticated) — `deploy/teardown.sh` deleted the smoke run's repository through `gh api`, which needs the `delete_repo` scope on the operator's own GitHub credential; the operator refused that scope outright, and the refusal is recorded as final. The teardown now authenticates to GitHub as the sandbox fine-grained PAT in `WILLIKINS_GITHUB_TOKEN`, through `curl --config -` on stdin exactly as the Doppler half does, sending the three headers the GitHub client sends — a capability already proved by the live write cycle, whose guard deletes a repository with the same token, and a blast radius the size of one throwaway organization rather than every repository the operator can administer. The smoke test's pre-flight drops the scope check for a presence check on both credentials, and nothing in the workspace shells out to `gh` for a write. Landed through Workflow `wf_b2605177-57f` (sonnet, verified by opus; gates green). The verify found a real leak the implementation had missed: both tokens sat in the *environment* every `curl` child inherited, which `ps -E` prints to anything running as the same user, while the script's header claimed the opposite; both are now copied into non-exported shell variables and `unset` before any child is spawned, the stubs record each child's environment as well as its argv, and the header says exactly what the defence does and does not cover. Fourteen teardown scenarios now, ten mutations run against them. So part B waits on nothing.
**Addendum:** 2026-09-16 (task 14, part B — the milestone is complete) — the live smoke run passed end to end against the operator's sandbox accounts in 30 seconds, and acceptance test 18 is met: the first apply read `names` `Computed`, created `repo`, `doppler`, `token` and `ci_secret`, and read the three root configs `Unchanged` (the amended wording, confirmed live); the second apply read every node `Unchanged` with `ci_secret` `Converged` and the `token` output `Unknown` carrying no value key; `workflows/rotate-service-token.yaml` was refused without approval and applied with it; `deploy/teardown.sh` removed both resources, dry run first; and the test's own leftover check, plus two independent reads afterwards, found the repository gone (`404`, and the whole sandbox organization empty) and the Doppler project unreadable. Three run ids, one journal, no credential in any output. The first attempt, earlier the same day, failed in four seconds during the *initial* plan and created nothing: `doppler.service_token.ensure`'s read listed tokens through `GET /v3/configs/config/tokens`, and on a fresh account Doppler answers `404` for a project the plan has not created yet, which the tool propagated as a hard `NotFound`. Fixed (Workflow `wf_8c292047-d33`, sonnet verified by opus, gates green at 1,709 then 1,714 tests): both token tools' *reads* now read that `404` as `Absent`, `rotate`'s `ensure` stays strict on purpose (a rotate that cannot list what it must revoke must not mint a replacement, pinned by its own test), a `400` from the same endpoint still refuses, and the code now argues why `Absent` is the only available reading rather than assuming it — `Foreign` would become `NameTaken` and refuse every ordinary plan, and `Absent` reaches no call an empty `200` did not already reach. The verify also pinned what the fix's prose had merely claimed (`ensure` still fails for real when the parent is still missing at apply time), added a live-write-cycle step that reads a token before its project exists, corrected an over-claim about what a Doppler `404` can distinguish, and pinned `ButlerError::Plan`'s new `PlanAttempt` at its call site: the first attempt's message said "re-planning failed" for a failure in the *initial* plan, which is now "planning failed", with the JSON shape unchanged. Why no offline gate caught the original defect, which is the durable lesson: the Doppler live write cycle creates its project before listing tokens, so it never met the `404`, and the fake answers `Absent` for a project it does not hold, so `tests/smoke_parity.rs` — whose two sides are both the fake — agreed with itself. Each provider crate's own live-against-fake parity test is what pins that class, and `smoke_parity.rs`'s header now says what it cannot see. Open, recorded rather than guessed: whether Doppler answers `404` or `403` for a project outside a service account's grant, which needs a deliberately under-granted second sandbox token to settle.
**Addendum:** 2026-09-16 (credential boundary, operator) — the operator's own `gh` CLI credential is never used for a write against their accounts, and never asked to carry a wider scope. Their words, on being asked for `delete_repo`: "That one is out of the question", and then: "I already provided a pat, scoped to the test org. On purpose. So a mistake could *not* wreck anything. I *never* allowed using my github cli set up to create or delete repositories." The teardown accordingly authenticates as `WILLIKINS_GITHUB_TOKEN`, whose blast radius is one throwaway organization, and no `gh` invocation remains in any file this workspace runs; a guard test enforces that. The same principle retires the secret-scanning push-protection bypass the coordinator had been using to land pushes blocked by synthetic provider-shaped literals in fixtures: disabling a security control with the operator's credential is the wrong end of the problem, so the literals are built at runtime from parts instead and a second guard test fails on any new one.
...
5. **Provider responses are a redaction boundary.** A `ToolError.message` is built from the
   ...
   own logging is never enabled. `tracing` output carries request method, the concrete path
   (a path carries no document text on this transport; `TraceLayer` records it at DEBUG,
   which the binary never emits), status, and duration, never a body or a header value.
   INFO is the ceiling: the binary installs no `EnvFilter` and does not read `RUST_LOG`
   (task 13); how an operator raises the level, and a sweep of rmcp's own debug and trace
   output once they can, is milestone 2c's exposure work together with the OAuth transport.
...
| `doppler.service_token.ensure` | `GET /v3/configs/config/tokens?project&config`; a listed token whose `name` matches -> `Present` with `token` `Unknown`; none, **and a `404` naming a project or config the plan has not created yet** (task 14, part B), -> `Absent` with `token` `Unknown`; a `400` or any other status still refuses. The list omits `key` and `access` | `Present` -> no call, `Unknown`; `Absent` -> `POST /v3/configs/config/tokens` with `project`, `config`, `name`, `access: "read"`; parse the response's `token.key` into `DopplerServiceToken`; drop the response; return it `Known` |
| `doppler.service_token.rotate` (new; inputs `config`, `name`; output `token`; key `config`, `name`; class `Destructive`) | always `Absent` with `token` `Unknown`, and its read tolerates the same not-yet-created `404` as `ensure`'s while its *ensure* stays strict (a rotate that cannot list what it must revoke must not mint a replacement) (a Destructive step must never plan as `NoOp`; the fake agrees and the two are pinned against each other) | for every listed token with that `name`, `DELETE /v3/configs/config/tokens/token` with body `project`, `config`, `slug`; then mint as above; `Known` output |
...
- `ApplyRefused { plan_id, principal, reason }`, one journaled refusal per kind-tagged apply
  error. The reason set is owned by `ApplyRefusedReason`'s doc in `willikins-journal`, not
  by this list: it began as `UnknownPlan`, `DocumentChanged`, `Drift`, `PlanExpired`,
  `ApprovalRequired`, `AlreadyApplied`, grew in task 10a (`PlanFailed`, `RunInProgress`
  among them) and again in task 13 (`RecordedInputUnreadable`, `ApplyPreparing`); every
  change is additive and a frozen pre-change journal must keep replaying.
...
18. **Live smoke (opt-in).** With `WILLIKINS_LIVE_TESTS=1` and sandbox credentials, the
    whole workflow **plans** against an account where nothing it names exists yet — every
    tool's read answers for a resource whose parent the same plan will create, and a
    provider's "no such parent" is an observation, never a refusal (task 14, part B) — and the
    positive fixture then applies with `repo`, `doppler`, `token` and `ci_secret` `Created` and
    the three root configs `Unchanged` (Doppler creates them with the project; amended
    2026-09-16, task 14 part A), a second apply is `Unchanged` throughout with `ci_secret`
    `Converged`, the rotation workflow is refused without approval and applies with it,
    and the teardown script removes the repository and the project, confirmed by a
    leftover check the script does not share. The recorded responses, redacted, refresh
    the mock fixtures through the two provider write cycles.
19. **Adversarial passes.** Two, recorded under `docs/research/`. The first, after tasks 4
    ...
    the trusted directory; replay or forge a `plan_id`; exhaust the blocking pool (bounded
    since task 13 at 64 concurrent tool calls, the rest refused `Busy`, so the attack is to
    reach the bound and show that `/healthz` and another principal still answer). Every
...
- **A synchronous core under an asynchronous server.** Every `plan` and `apply` occupies a
  blocking thread for its duration. Pass 2 showed the real failure mode: once the pool is
  full, unrelated principals' calls queue behind stuck ones while `/healthz`, which uses no
  blocking thread, keeps answering, so the deployment's own health check cannot see it.
  Interim answer (task 13): at most 64 concurrent tool calls hold a blocking thread and the
  rest are refused with a kind-tagged `Busy`, never queued. The real fix is an async `Tool`,
...
- Doppler layout (operator, 2026-09-14): secrets shared by many projects (App Store Connect
  ...
  Enterprise feature per docs.doppler.com/docs/config-inheritance, fetched 2026-09-14; the
  operator confirmed the same day that every one of their workplaces, the test one
  included, is on a Team plan, so milestone 3's live cycle can exercise both tools).
