...
**Addendum:** 2026-09-16 (task 14, part A) — the live smoke run split in two so nothing is created before it can be deleted. Part A (Workflow `wf_ba182dd5-636`, opus; gates green at 1,704 tests) ran the two read-only probes with the sandbox file sourced in the same command (GitHub: 3 pass, 1 skip, the org still has no repository; Doppler: the credential authenticated, the project checks 404 by design in the empty workplace) and wrote acceptance test 18 as code: `crates/willikins-cli/tests/live_smoke.rs`, behind a `live-tests` feature plus `WILLIKINS_LIVE_TESTS=1`, `#[ignore]`d, driving the real `willikins` binary through the positive fixture twice (`slug=willikins-smoke`, `org=Willikins-Test`, one persistent `--journal`, `--principal smoke-operator`), the rotation refused without `--approve` and applied with it, `deploy/teardown.sh` as a dry run then `--yes`, and a leftover check independent of the script; a pre-flight inside the test refuses to create anything unless `gh`'s token carries the `delete_repo` scope the script's `gh api -X DELETE` needs, `jq` is on `PATH`, and the repository does not already exist. Its twin `tests/smoke_parity.rs` drives the same four invocations against the fake catalog in every gate, so the live run is the first time only the providers differ. Two corrections to this plan's words, found by driving the binary: on the first apply `configs[dev]`, `configs[stg]` and `configs[prd]` read `Unchanged`, not `Created`, because Doppler creates the three root configs with the project and `doppler.config.ensure` re-observes them `Present` (`changed: false`), exactly as the Doppler section above predicts (the fake agrees); and on the second apply `ci_secret` reads `Converged`, not `Unchanged`, because its `value` input is `Unknown` (a minted token cannot be re-read) and the planned action is a no-op. Acceptance test 18 is amended in place below. Part B, the live run itself, waits on the operator granting `gh` the `delete_repo` scope (`gh auth refresh -h github.com -s delete_repo`); the recorded fixtures are refreshed by the two provider write cycles, not by the smoke test, and their 2026-09-14 verification stands unless part B's run shows a new shape.
