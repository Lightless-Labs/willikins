...
- The operator's habit of sharing Apple distribution certificates across projects is not a preference to
  design away, and revocation is destructive across every project at once. **But Apple's stated per-team
  limit is wrong, corrected by observation 2026-09-22 — see the correction below.** (source: <https://developer.apple.com/support/certificates/>)
  ...
  - **CORRECTION, 2026-09-22, operator observation.** The support page quoted above says "only one type of
    each distribution certificate (with the exception of Developer ID certificates) is allowed per team".
    That is false as written. The operator reported holding **three simultaneous "Apple Distribution (All
    Platforms)" certificates** on their live team: *"Until ten seconds ago, I had 3 'All platforms'
    distribution certificates."* The widely repeated third-party figure of three, previously recorded here
    as **(unverified)** and not relied on, matches what was observed; Apple's own page does not. Treat the
    published limit as unreliable and the real cap as at least three, exact value unknown.
  - **Consequence for the design.** "Which distribution certificate" is a REAL choice, not a formality.
    A profile tool cannot assume the team has exactly one candidate and pick it implicitly; it must take
    the certificate as an explicit port (`certificateType` plus `displayName`, or the certificate id) and
    must refuse rather than guess when a read returns more than one match. An earlier reading of this note
    concluded the opposite and would have produced a tool that silently signed with whichever certificate
    Apple happened to list first.
...
- The per-team caps on certificate types. The *distribution* cap is now known to be at least three, not one
  (operator observation 2026-09-22, above), so Apple's published statement is unreliable and the exact cap
  is unknown. Caps for development, pass type and APNs types remain third-party and **(unverified)**.
